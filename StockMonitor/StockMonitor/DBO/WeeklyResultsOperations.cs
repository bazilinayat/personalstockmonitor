...
namespace StockMonitor.DBO
{
    ...
    public class WeeklyResultsOperations
    {
        ...
        public async Task<List<WeeklyResults>> GetWeeklyResultsBasedOnRemarkDates(DateTime from, DateTime to)
        {
            ...
            List<WeeklyRemarks> remarks = await _connection.Table<WeeklyRemarks>().Where(o => o.RemarkDate >= fromDate && o.RemarkDate <= toDate && o.IsChecked).ToListAsync();
            if (remarks.Count == 0)
                return null;
        ...
        public async Task<string?> SaveWeeklyResultAsync(WeeklyResults details)
        {
